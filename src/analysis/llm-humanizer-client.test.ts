...
import {
  HumanizerClient,
  type HumanizerClientOptions,
} from "./llm-humanizer-client";
import { LlmAnalysisError } from "./llm-analysis-client";
...
function assembledRequest(): AssembledHumanizerRequest {
  return {
    ...
    inputData: JSON.stringify({ text: copyrightedSource }),
    outputSchema: {},
    storyIds: ["story-1"],
    tokens: {
      instructions: 10,
...
function baseOptions(
  overrides: Partial<HumanizerClientOptions> = {},
): HumanizerClientOptions {
  return {
    provider: "openai",
    openai: {
      apiKey: secretKey,
...
describe("HumanizerClient (openai provider, Responses API)", () => {
  ...
  it("retries transient errors with bounded exponential delays", async () => {
    ...
    await expect(
      openaiClient(createResponse, { sleep }).humanize(assembledRequest()),
  ...
  it("returns refusals as an explicit outcome", async () => {
    ...
    await expect(
      openaiClient(async () => refusalResponse).humanize(assembledRequest()),
  ...
  it("returns incomplete and failed responses as explicit outcomes", async () => {
    ...
    await expect(
      openaiClient(async () => incomplete).humanize(assembledRequest()),
    ).resolves.toMatchObject({
      kind: "incomplete",
    ...
    await expect(
      openaiClient(async () => failed).humanize(assembledRequest()),
  ...
  it("logs only classified metadata, never credentials or source bodies", async () => {
    ...
    await openaiClient(async () => response(), { logger }).humanize(
      assembledRequest(),
    );
...
describe("HumanizerClient (openrouter provider, Chat Completions API)", () => {
  ...
  it("retries transient errors with bounded exponential delays", async () => {
    ...
    await expect(
      openRouterClient(createCompletion, { sleep }).humanize(
        assembledRequest(),
      ),
    ).resolves.toMatchObject({ kind: "completed" });
  ...
  it("does not retry terminal API errors", async () => {
    ...
    await expect(
      openRouterClient(createCompletion).humanize(assembledRequest()),
    ).rejects.toMatchObject({
      name: "LlmAnalysisError",
      retryable: false,
      status: 400,
  ...
  it("returns refusals as an explicit outcome", async () => {
    ...
    await expect(
      openRouterClient(async () => refusalResponse).humanize(
        assembledRequest(),
      ),
    ).resolves.toMatchObject({
      kind: "refusal",
  ...
  it("returns a truncated completion as an incomplete outcome", async () => {
    ...
    await expect(
      openRouterClient(async () => truncated).humanize(assembledRequest()),
  ...
  it("logs only classified metadata, never credentials or source bodies", async () => {
    ...
    await openRouterClient(async () => chatCompletion(), { logger }).humanize(
      assembledRequest(),
    );
...
describe("LlmAnalysisError re-export", () => {
  it("is the error class used to classify humanizer failures", () => {
    const error = new LlmAnalysisError("known", false, null, null);
    expect(error.name).toBe("LlmAnalysisError");
  });
});
